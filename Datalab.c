 ...
 */
int isTmax(int x) {
 int result, is_minusone;
 result = !((~x) ^ (x+1));
 is_minusone = !(~x);
 return result & (!is_minusone);
}
//2
/*
...
int evenBits(void) {
  int tmp;
  tmp=0x55; // 0101 0101
  tmp = tmp | (tmp << 8); // 0101 0101 0101 0101
  tmp = tmp | (tmp << 16); // 0101 0101 0101 0101 0101 0101 0101 0101
  return tmp;
}
//3
/* 
...
int isGreater(int x, int y) {
  int equal, diff, diffMSbit, xMSbit, yMSbit, sameSign;
  equal = !(x ^ y);
  diff = x + ~y + 1;
  diffMSbit = (diff >> 31) & 0x1;
  xMSbit = (x >> 31) & 0x1;
  yMSbit = (y >> 31) & 0x1;
  sameSign = !(xMSbit ^ yMSbit);
  return (yMSbit & (!sameSign)) | ((!diffMSbit) & sameSign & (!equal)) ;
}
//7
/*
...
int twosComp2SignMag(int x) {
  int sign, abs_val, mag_val;
  sign = x>>31;
  abs_val = (x ^ sign) + (sign & 1);
  mag_val = abs_val | (sign << 31);
  return mag_val;
}
//10
/*
 ...
 */
int isPower2(int x) {
  int sign, oneBit, is_Zero;
  sign = x >> 31;
  oneBit = x & (~x + 1);
  is_Zero = !x;          
  return !((sign | is_Zero) | (oneBit ^ x));
}
